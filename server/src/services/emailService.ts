...
export const sendOTP = async (to: string, otp: string) => {
    try {
        ...
        const info = await transporter.sendMail({
            from: '"SplitEase" <verify@spendsplite.com>',
            to,
            subject: "Your Verification Code - SplitEase",
            text: `Your verification code is: ${otp}`,
            html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #0f172a;">Welcome to SplitEase!</h2>
...
export const sendPasswordResetOTP = async (to: string, otp: string) => {
    try {
        ...
        const info = await transporter.sendMail({
            from: '"SplitEase" <verify@spendsplite.com>',
            to,
            subject: "Password Reset Code - SplitEase",
